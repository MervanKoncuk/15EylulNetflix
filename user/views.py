...
def userRegister(request):
    if request.method == "POST":
        ...
        sifre2 = request.POST.get('sifre2') # Alternatif
        ...
        if sifre == sifre2:
            if User.objects.filter(username = kullanici).exists():
                ...
                user = User.objects.create_user(
                    username = kullanici, email = email, password = sifre
                )
...
def userLogin(request):
    if request.method == 'POST':
        ...
        user = authenticate(
            request, username = kullanici, password = sifre
        )
        if user is not None:
            login(request, user)
...
def create(request):
    ...
    if request.method == "POST":
        try:
            print("Profil Sayısı:", Profile.object.filter(olusturan = request.user).count())
            ...
            messages.error(request, 'Bilinmeyen bir hata çıktı. Tekrar deneyiniz. Düzelmediği takdirde bizimle iletişime geçebilirsiniz')
